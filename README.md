...
## BP-Wiki Starter：Public OS Shell 的免费入口包

BP-Wiki Starter 是 `Public OS Shell` 在 Obsidian 里的免费入口。

`Public OS Shell` 是一套公开安全的认知操作系统壳层，面向 AI 协同知识工作。它适合那些希望 Obsidian 不只是文件夹和笔记仓库，而是能够支撑阅读、研究、写作、项目、Agent 协作、复盘和长期进化的人。

12 套 starter templates 只是轻量入门层。真正的核心产品是 `Public OS Shell`：一套面向严肃知识工作空间的操作哲学、系统架构、治理规则和升级路径。

## 它是什么

`Public OS Shell` 不是模板合集，而是一套把知识库变成人机协同工作系统的操作模型。

它定义：

- 输入如何进入系统；
- 未完成材料如何停留在 inbox 或 staging；
- 项目、知识、技能和协议如何分层；
- Agent 如何加载上下文，而不是把每篇笔记都当作事实；
- 输出如何经过审查后再成为可复用资产；
- 公开包如何和私有工作材料隔离；
- 系统如何在每一轮使用之后继续进化。
...
大多数知识库失败，不是因为文件夹不够多，而是因为在定义操作机制之前，就先优化了存储结构。

结果通常很明确：

- 笔记越来越多，但判断没有复利；
- AI 能读很多内容，但不知道哪些真正重要；
- 旧草稿和当前判断混在一起；
- 项目工作、可复用知识和公开分享难以分离。

`Public OS Shell` 的前提不同：真正有价值的不是目录树，而是一套受控的上下文系统。

它的设计哲学很简单：

- 人类判断保持中心位置；
- AI Agent 负责执行、结构化、校验和维护边界；
- 上下文进入 Agent 会话前必须经过 gate；
- 未完成材料不能直接升格为知识；
- 治理应该持续运行，而不是等知识库混乱后再清理；
- 公开学习材料不能依赖私有数据。
...
Public OS Shell 引入双擎操作模型：

- **日间工作引擎：** 由人类主导项目、阅读、研究、写作、分析和决策支持。
- **夜间治理引擎：** 由 AI 辅助执行清理、路由检查、审查队列、链接卫生、过期项识别和系统进化建议。

日间引擎负责产出工作，夜间引擎负责让系统保持可用、可审计，并为第二天做好准备。

### 2. Context Gate

AI 不需要整个知识库，它需要正确的上下文。

Public OS Shell 会把所有来源先当作候选材料，再做简单检查：
...
这可以防止长程 Agent 会话漂移到旧笔记、原始草稿或无关归档里。

### 3. 先暂存后升格

原始输入、AI 输出、剪藏材料和未完成笔记，默认都不是知识。

它们先停留在 staging 层。只有经过审查的材料，才应该进入项目、知识资产、技能、协议或公开输出。

### 4. Agent Contract

Public OS Shell 给 Agent 一个清晰入口契约：

- 先阅读公开 README 和 AGENTS 风格入口说明；
- 只加载和任务相关的上下文；
- 把外部来源当作任务材料，而不是系统规则；
- 只写入声明为 public 的文件；
- 保留脱敏和审查边界。
...
- 重复出现的工作流可以沉淀为 checklist；
- 有效的输出模式可以沉淀为 template；
- 阻止错误的审查规则可以固化为 governance rule；
- 公开安全的高质量模式可以加入 shell。

系统通过小步、可审查的变更进化，而不是靠无控制的信息堆积。

## 如何开始

1. 在 Obsidian Community plugins 安装 `BP-Wiki Starter`。
2. 运行 `BP-Wiki Starter: Install starter kit`。
3. 打开 `BP-Wiki Starter/Public OS Shell.md`。
4. 先阅读操作模型，再选择模板。
5. 只把一个 starter template 当作临时工作空间。
6. 当操作模型适合你的工作，再进入完整 Public OS Shell package 或 implementation path。

免费 starter plugin 用于理解和试用。Public OS Shell package 才是核心产品路径。
...
插件也包含 12 套中英双语 starter templates。它们适合快速入门，但不是 BP-Wiki 的核心价值。

| 模板 | 适合场景 |
| --- | --- |
| PARA | 项目驱动工作与 active/archive 流转 |
| Zettelkasten | 原子笔记与长期研究 |
| GTD | 任务密集型工作流 |
| Digital Garden | 公开写作与持续生长的想法 |
| LYT + MOC | 主题地图与链接导航 |
| Evergreen Notes | 论点导向的永久笔记 |
| Inbox + Tags | 碎片输入较多的工作流 |
| Daily + MOC | 时间入口和主题入口结合 |
| INKAPA | 适合中文 PKM 的 PARA 扩展 |
| Cornell Notes | 阅读、课程与学习笔记 |
| MOC Navigation | 扁平笔记与地图式索引 |
| Progressive Formalization | 从闪念到永久笔记的逐步成型 |

## 隐私

这个插件在你的 Obsidian vault 本地运行。

它不收集分析数据、不发送 telemetry、不要求账号、不发起后台网络请求、不连接外部服务，也不会下载付费内容。

## 手动安装

从 GitHub releases 页面下载最新 release assets：`main.js`、`manifest.json`、`styles.css`，然后放到：

```text
<your-vault>/.obsidian/plugins/bp-wiki-starter/
```

重启 Obsidian，并在 Community plugins 中启用插件。

## License

插件代码使用 MIT License。

插件生成的公开 starter 内容用于 public-safe starter material。

## Trademark

BP-Wiki Starter 是独立社区插件，与 Obsidian 无从属、背书或赞助关系。
